...
class GRU4Rec:
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=100):
        ...
        if self.predict is None or self.predict_batch != batch:
            ...
            h = torch.zeros((batch, self.model.gru.hidden_size))
            in_idxs = torch.tensor(self.itemidmap[input_item_ids], dtype=torch.long).unsqueeze(1)
            if predict_for_item_ids is not None:
                iIdxs = torch.tensor(self.itemidmap[predict_for_item_ids], dtype=torch.long).unsqueeze(1)
                preds, _ = self.model(in_idxs, h)
                preds = preds[:, iIdxs.squeeze(1)].detach().numpy().T
                return pd.DataFrame(data=preds, index=predict_for_item_ids)
            else:
                preds, _ = self.model(in_idxs, h)
                preds = preds.detach().numpy().T
    ...
    def symbolic_predict(self, X, Y, M, items, batch_size):
        if not self.constrained_embedding:
            M = None
        H = [Variable(torch.zeros(batch_size, self.model.gru.hidden_size), requires_grad=False) for _ in range(len(self.model.gru.layers))]
        if items is not None:
            X = Variable(X, requires_grad=False)
            Y = Variable(Y, requires_grad=False)
            M = Variable(M, requires_grad=False) if M is not None else None
